...
- how to install `pynxtools-xrd`
- how to install `pynxtools-xrd` together with NOMAD
...
=== "uv"
    ...
    ```bash
    uv pip install git+https://github.com/FAIRmat-NFDI/pynxtools-xrd.git
    ```

=== "pip"

    ```bash
    pip install git+https://github.com/FAIRmat-NFDI/pynxtools-xrd.git
    ```

### How to install `pynxtools-xrd` with NOMAD

To use `pynxtools-xrd` with NOMAD, simply install it in the same environment as the `nomad-lab` package. NOMAD will recognize `pynxtools` (and it `pynxtools-xrd` reader plugin) as a plugin automatically. In addition, NOMAD will install a schema for NeXus application definitions.

## Start using `pynxtools-xrd`

That's it! You can now use `pynxtools-xrd`!
